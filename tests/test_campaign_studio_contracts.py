...
class Complete99_Content{public static function find_translation_post_id($key,$language,$public_only){if($key!=='store'||$language!=='he'||$public_only!==true)throw new Exception('Unexpected storefront ownership lookup');return 2;}}
...
""".replace("__CAMPAIGN_PATH__", campaign_path)
        ...
        self.assertEqual("pass", result["capacity"], result)
        self.assertEqual("active", result["finalState"], result)
        self.assertEqual(3, result["finalGeneration"], result)

    def test_fresh_lifecycle_bootstrap_requires_the_full_durable_transition(self) -> None:
        install = self.php.split("private static function install_lifecycle_reservation", 1)[1].split(
            "/** Deterministic advisory identity", 1
        )[0]
        capacity = self.php.split("private static function lifecycle_capacity_reservation", 1)[1].split(
            "/** Exact per-table reserve", 1
        )[0]
        resume = self.php.split("private static function resume_schedules_fenced", 1)[1].split(
            "/** Prevent public slot HTML", 1
        )[0]
        self.assertIn("repair_legacy_bootstrap_lifecycle", install)
        self.assertIn("self::$activation_recovery_mode ? 'suspending' : 'active'", install)
        self.assertIn("lifecycle_prior_inactive_receipt_requirement", capacity)
        self.assertIn("lifecycle_prior_inactive_receipt_requirement", resume)
        self.assertNotIn("lifecycle_fresh_install", resume)
        self.assertNotIn("lifecycle_fresh_install_nonempty", resume)

    @unittest.skipUnless(shutil.which("php"), "PHP is required")
    def test_fresh_and_legacy_lifecycle_bootstrap_lineage_executes(self) -> None:
        campaign_path = str(CAMPAIGNS).replace("\\", "/").replace("'", "\\'")
        script = r"""
define('ABSPATH',__DIR__);define('ARRAY_A','ARRAY_A');define('DATABASE_TYPE','sqlite');
class WP_Error{private $code;private $data;public function __construct($code,$message='',$data=array()){$this->code=$code;$this->data=$data;}public function get_error_code(){return $this->code;}public function get_error_data(){return $this->data;}public function get_error_message(){return $this->code;}}
function is_wp_error($v){return $v instanceof WP_Error;}function wp_json_encode($v,$flags=0){return json_encode($v,$flags);}function wp_cache_delete($key,$group=''){return true;}
class Complete99_Ops{public static function table_names(){return array('commands'=>'wp_c99_commands','mutation_receipts'=>'wp_c99_mutation_receipts','audit_events'=>'wp_c99_audit_events','issues'=>'wp_c99_issues','issue_events'=>'wp_c99_issue_events','memberships'=>'wp_c99_memberships','budgets'=>'wp_c99_budgets');}}
class C99BootstrapWpdb{public $prefix='wp_';public $options='wp_options';public $last_error='';public $lifecycle=null;public $occupied=false;public function prepare($q,...$a){return array('query'=>$q,'args'=>$a);}private function q($p){return is_array($p)?$p['query']:(string)$p;}private function a($p){return is_array($p)?$p['args']:array();}public function get_results($p,$f=null){$q=$this->q($p);if(false!==strpos($q,'FROM wp_options'))return null===$this->lifecycle?array():array(array('option_id'=>7,'option_name'=>'complete99_campaign_lifecycle_reservation_v1','option_value'=>$this->lifecycle,'autoload'=>'no'));if(false!==strpos($q,'c99_campaign_provider_receipts')||false!==strpos($q,'c99_audit_events'))return $this->occupied?array(array('id'=>1)):array();return array();}public function query($p){$q=$this->q($p);$a=$this->a($p);if(false!==strpos($q,'INSERT INTO wp_options')){$this->lifecycle=(string)($a[1]??'');return 1;}if(false!==strpos($q,'UPDATE wp_options SET option_value=')){$this->lifecycle=(string)($a[0]??'');return 1;}return 1;}}
$wpdb=new C99BootstrapWpdb();require '__CAMPAIGN_PATH__';function bm($n){$m=new ReflectionMethod('Complete99_Campaigns',$n);$m->setAccessible(true);return $m;}function bp($n){$p=new ReflectionProperty('Complete99_Campaigns',$n);$p->setAccessible(true);return $p;}
$requirement=bm('lifecycle_prior_inactive_receipt_requirement');$install=bm('install_lifecycle_reservation');$canonical=bm('canonical_json');bp('activation_recovery_mode')->setValue(null,true);bp('lifecycle_role_lock_owned')->setValue(null,true);bp('worker_execution_fence_owned')->setValue(null,true);
$shape=function($value){return is_wp_error($value)?$value->get_error_code():$value;};$requirements=array('active1'=>$shape($requirement->invoke(null,'active',1)),'active2'=>$shape($requirement->invoke(null,'active',2)),'active3'=>$shape($requirement->invoke(null,'active',3)),'active4'=>$shape($requirement->invoke(null,'active',4)),'inactive1'=>$shape($requirement->invoke(null,'inactive',1)),'inactive2'=>$shape($requirement->invoke(null,'inactive',2)));
$install->invoke(null);$fresh=json_decode($wpdb->lifecycle,true);$legacy=array('changedAt'=>'2026-08-12T00:00:00Z','generation'=>2,'schemaVersion'=>'complete99-campaign-lifecycle-reservation/v1','state'=>'active');$wpdb->lifecycle=$canonical->invoke(null,$legacy);$install->invoke(null);$repaired=json_decode($wpdb->lifecycle,true);$wpdb->lifecycle=$canonical->invoke(null,$legacy);$wpdb->occupied=true;$collision='';try{$install->invoke(null);}catch(Throwable $error){$collision=$error->getMessage();}
echo json_encode(array('requirements'=>$requirements,'fresh'=>$fresh,'repaired'=>$repaired,'collision'=>$collision),JSON_THROW_ON_ERROR);
""".replace("__CAMPAIGN_PATH__", campaign_path)
        result = json.loads(subprocess.run(
            ["php", "-r", script], cwd=ROOT, capture_output=True, text=True,
            encoding="utf-8", timeout=30, check=True,
        ).stdout)
        self.assertIsNone(result["requirements"]["active1"], result)
        self.assertEqual("complete99_campaign_lifecycle_bootstrap_unproven", result["requirements"]["active2"], result)
        self.assertEqual({"receiptGeneration": 1, "inactiveGeneration": 2}, result["requirements"]["active3"], result)
        self.assertEqual({"receiptGeneration": 2, "inactiveGeneration": 3}, result["requirements"]["active4"], result)
        self.assertEqual("complete99_campaign_lifecycle_bootstrap_unproven", result["requirements"]["inactive1"], result)
        self.assertEqual({"receiptGeneration": 1, "inactiveGeneration": 2}, result["requirements"]["inactive2"], result)
        self.assertEqual({"state": "suspending", "generation": 1}, {"state": result["fresh"]["state"], "generation": result["fresh"]["generation"]}, result)
        self.assertEqual({"state": "suspending", "generation": 3}, {"state": result["repaired"]["state"], "generation": result["repaired"]["generation"]}, result)
        self.assertEqual("complete99_campaign_lifecycle_legacy_repair_collision", result["collision"], result)
