...
if (in_array($path, ['/mobile/he/', '/mobile/en/'], true) && ($_SERVER['REQUEST_METHOD'] ?? '') === 'GET') {
    ...
    $form_path = $locale === 'en' ? '/en/request-proposal/' : '/request-proposal/';
    echo '<!doctype html><html lang="en"><meta charset="utf-8"><title>Local 390px enquiry preview</title><body><p>Local responsive test. Simulated responses only.</p><iframe title="390px enquiry" src="' . $form_path . '" style="width:390px;height:850px;border:0"></iframe></body></html>';
    exit;
}
if (in_array($path, ['/wp-admin/admin-post.php', '/request-proposal/', '/en/request-proposal/'], true) && ($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    $status = (string) ($_POST['_test_reply'] ?? '400');
    if ($status === '200') {
        $language = ($_POST['language'] ?? '') === 'en' ? 'en' : 'he';
        header('Location: ' . ($language === 'en' ? '/en/request-proposal/' : '/request-proposal/') . '?c99_sent=1', true, 303);
    } else {
...
if (!in_array($path, ['/preview/he/', '/preview/en/', '/request-proposal/', '/en/request-proposal/'], true) || ($_SERVER['REQUEST_METHOD'] ?? '') !== 'GET') { http_response_code(404); exit; }
ob_start();
require __DIR__ . '/lead-form-contract.php';
$html = ob_get_clean();
require dirname(__DIR__, 2) . '/editorial-release/plugin/group-enquiry.php';
if (in_array($path, ['/request-proposal/', '/en/request-proposal/'], true)) { $html = c99_editorial_enquiry_form($html, $path); }
