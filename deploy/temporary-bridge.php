...
add_action(
	...
	static function () {
		...
		$read_lock = static function () use ( $lock_option ) {
			$lock = get_option( $lock_option, array() );
			return is_array( $lock ) ? $lock : array();
		};

		$acquire_lock = static function ( $deployment_id, $phase = 'reserved' ) use ( $lock_option ) {
			$lock = array(
			...
			if ( ! add_option( $lock_option, $lock, '', false ) ) {
				...
				if ( $deployment_id === $current_id && 'reserved' === $current_phase ) {
					if ( 'reserved' !== $lock['phase'] ) {
						$current['phase']      = $lock['phase'];
						$current['updated_at'] = time();
						update_option( $lock_option, $current, false );
					}
					return $current;
				}
				return new WP_Error(
		...
		$release_lock = static function ( $deployment_id ) use ( $lock_option ) {
			$current = get_option( $lock_option, array() );
			if ( ! is_array( $current ) || $deployment_id !== (string) ( $current['deployment_id'] ?? '' ) ) {
				return false;
			}
		...
		$set_state_phase = static function ( $state_dir, $deployment_id, $phase, $extra = array() ) use ( $lock_option, $write_state_file ) {
			...
			if ( is_array( $lock ) && $deployment_id === (string) ( $lock['deployment_id'] ?? '' ) ) {
				$lock['phase']      = $state['phase'];
				$lock['updated_at'] = $state['updated_at'];
				update_option( $lock_option, $lock, false );
			}
			return $state;
		};

		$make_test_lock_stale = static function ( $deployment_id ) use ( $config, $lock_option ) {
			...
			$lock['updated_at'] = time() - (int) $config['recovery_lease_seconds'] - 5;
			update_option( $lock_option, $lock, false );
			return true;
		};
		...
		register_rest_route(
			...
			array(
				...
				'callback'            => static function () use ( $config, $bootstrap_filesystem, $auto_update_enabled, $acquire_lock, $verify_transactional_storage, $capture_database_state ) {
					global $wp_filesystem;
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'permission_callback' => $permission,
				'callback'            => static function ( WP_REST_Request $request ) use ( $config, $bootstrap_filesystem, $state_directory, $read_lock, $directory_sha256, $capture_database_state ) {
					global $wp_filesystem;
					$filesystem = $bootstrap_filesystem();
					if ( is_wp_error( $filesystem ) ) {
						return $filesystem;
					...
					$recovery_ready = $lock_owned
						...
						&& in_array( $phase, array( 'locked', 'prepared', 'installing', 'rolling_back', 'committing' ), true );
					return array(
						'deployment_id'    => $deployment_id,
						'phase'            => $phase,
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'permission_callback' => $permission,
				'callback'            => static function ( WP_REST_Request $request ) use ( $config, $bootstrap_filesystem, $state_directory, $auto_update_enabled, $purge_caches, $acquire_lock, $release_lock, $write_state_file, $set_state_phase, $make_test_lock_stale, $directory_sha256, $verify_transactional_storage, $capture_database_state, $encrypt_database_state, $decrypt_database_state ) {
					global $wp_filesystem;
					...
					$lock = $acquire_lock( $deployment_id, 'locked' );
					if ( is_wp_error( $lock ) ) {
						return $lock;
					}
					$transactional_storage = $verify_transactional_storage();
					if ( is_wp_error( $transactional_storage ) ) {
						$release_lock( $deployment_id );
						return $transactional_storage;
					}
					...
					try {
						$install_response = ( static function () use ( $temp, $plugin_path, $version, $was_active, $activate, $config, $deployment_id, $actual, $slug, $purge_caches ) {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'permission_callback' => $permission,
				'callback'            => static function ( WP_REST_Request $request ) use ( $config, $bootstrap_filesystem, $state_directory, $purge_caches, $read_lock, $set_state_phase, $make_test_lock_stale, $directory_sha256, $restore_database_state, $decrypt_database_state ) {
					global $wp_filesystem;
					$filesystem = $bootstrap_filesystem();
					if ( is_wp_error( $filesystem ) ) {
						return $filesystem;
					...
					foreach ( array( $restore_stage, $displaced_dir ) as $swap_path ) {
						if ( $wp_filesystem->exists( $swap_path ) && ! $wp_filesystem->delete( $swap_path, true ) ) {
							return new WP_Error( 'c99_rollback_swap_cleanup', 'A prior rollback swap directory could not be removed.', array( 'status' => 500 ) );
						}
					}
					if ( $had_plugin ) {
					...
					if ( $wp_filesystem->exists( $target_dir ) && ! @rename( $target_dir, $displaced_dir ) ) {
						$set_state_phase( $state_dir, $deployment_id, 'rollback_failed' );
		...
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'permission_callback' => $permission,
				'callback'            => static function ( WP_REST_Request $request ) use ( $config, $bootstrap_filesystem, $state_directory, $purge_caches, $read_lock, $release_lock, $set_state_phase ) {
					global $wp_filesystem;
					$filesystem = $bootstrap_filesystem();
					if ( is_wp_error( $filesystem ) ) {
						return $filesystem;
					...
					if ( $wp_filesystem->exists( $state_file ) ) {
						if ( ! $lock_owned ) {
							return new WP_Error(
								'c99_finalize_lock',
								'The deployment does not own the mutation lock.',
								array( 'status' => 409 )
							);
						}
						$state = json_decode( $wp_filesystem->get_contents( $state_file ), true );
						...
						if ( in_array( $phase, array( 'installed', 'rolled_back', 'commit_failed', 'committing' ), true ) ) {
							$committing = $set_state_phase( $state_dir, $deployment_id, 'committing' );
						...
						$lock_updated_at = (int) ( $lock['updated_at'] ?? $lock['started_at'] ?? 0 );
						$lock_age = 0 < $lock_updated_at ? max( 0, time() - $lock_updated_at ) : 0;
						if ( $lock_age < (int) $config['recovery_lease_seconds'] ) {
							return new WP_Error(
								'c99_finalize_recovery_lease',
								'The interrupted deployment lock is not yet eligible for recovery.',
								array( 'status' => 409 )
							);
						}
